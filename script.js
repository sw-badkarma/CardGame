...
function loadCloudProgress(uid) {
  get(ref(db, `users/${uid}/save`)).then((snapshot) => {
    if (snapshot.exists()) {
      ...
      el('lobby-xp').textContent = playerXP;
...
document.getElementById('btn-close-auth').onclick = () => {
  authOverlay.classList.remove('active');
  setTimeout(() => authOverlay.classList.add('hidden'), 250);
  const warn = document.getElementById('offline-warning-overlay');
  warn.classList.remove('hidden'); setTimeout(() => warn.classList.add('active'), 20);
};

document.getElementById('btn-close-offline-warning').onclick = () => {
  const warn = document.getElementById('offline-warning-overlay');
  warn.classList.remove('active'); setTimeout(() => warn.classList.add('hidden'), 250);
};
...
function generateStageData(lvl) {
  let ascMod = 1 + (ascensionLevel * 0.2); // Creștere dificultate în Ascensiune
...
function getAndValidatePlayerName() {
  const rawInput = el('player-name-input').value.trim();
  if (!rawInput) { playerName = "Erou"; return true; }
  const regex = /^[a-zA-Z0-9ăîâșțĂÎÂȘȚ]{3,10}$/;
  if (!regex.test(rawInput)) { el('lobby-status').textContent = "Numele 3-10 caractere!"; return false; }
  playerName = rawInput;
  return true;
}

el('btn-open-talents').onclick = () => {
  el('talent-xp-count').textContent = playerXP;
  renderTalents();
  ...
  Object.keys(TALENTS_DB).forEach(key => {
    ...
    div.querySelector('.btn-buy').onclick = () => {
      if (!isMax && playerXP >= t.cost) {
        ...
        el('talent-xp-count').textContent = playerXP; el('lobby-xp').textContent = playerXP;
...
el('btn-emotes').onclick = () => el('emotes-popover').classList.toggle('hidden');

document.querySelectorAll('.emote-btn').forEach(btn => {
  btn.onclick = () => {
    const emoteStr = btn.textContent;
    el('emotes-popover').classList.add('hidden');
...
function startTurnTimer() {
  ...
  turnTimeLeft = 15;
  el('turn-timer').textContent = `⏳ ${turnTimeLeft}s`;
...
function startSoloGame(lvl, coins, deck, relics) {
  ...
  el('p1-name').textContent = playerName; el('player-avatar').textContent = selectedAvatar;
  el('p1-coins-wrap').classList.remove('hidden'); el('p1-coins').textContent = playerCoins;

  initSoloStage(currentStageLevel);
  switchScreen('lobby-overlay', 'game-container');
  el('shop-overlay').classList.add('hidden'); el('event-overlay').classList.add('hidden'); el('pause-overlay').classList.add('hidden');
  el('room-id-display').textContent = ascensionLevel > 0 ? `ASC ${ascensionLevel}` : "SOLO";
...
function initSoloStage(lvl) {
  ...
  el('p2-name').textContent = enemyData.name; el('enemy-avatar').textContent = enemyData.icon;
  if (el('stage-display')) {
  ...
  el('weather-display').textContent = currentWeather.name;
  el('arena').className = `battle-arena biome-${enemyData.biome}`;
...
function triggerRandomEvent() {
  ...
  if (isHealEvent) {
    el('event-title').textContent = "⛲ Fântâna Stăveche";
    el('event-desc').textContent = "Ai găsit o fântână magică ascunsă în umbra pădurii.";
    container.appendChild(createEventBtn("🥤 Beu apa binecuvântată (+20 HP Max)", () => {
      soloState.p1.maxHp += 20; soloState.p1.hp += 20;
      el('event-overlay').classList.remove('active'); setTimeout(() => el('event-overlay').classList.add('hidden'), 250);
      openShop();
    }));
    ...
    el('event-title').textContent = "🧙‍♂️ Negustorul Umbrelor";
    el('event-desc').textContent = "O figură misterioasă îți propune un târg...";
...
el('btn-create-room').onclick = () => {
  ...
  multiSubMode = document.querySelector('input[name="multi-type"]:checked').value;
  const roomId = Math.floor(1000 + Math.random() * 9000).toString();
  currentRoomId = roomId; playerRole = 'p1';
  el('p1-name').textContent = playerName; el('player-avatar').textContent = selectedAvatar;
  el('p1-coins-wrap').classList.add('hidden');
...
el('btn-join-room').onclick = () => {
  ...
  if (code.length !== 4) { el('lobby-status').textContent = "Introdu cod din 4 cifre!"; return; }
...
function listenToRoom(roomId) {
  onValue(ref(db, `rooms/${roomId}`), (snapshot) => {
    ...
    if (data.status === 'in_game') {
      ...
      if (data.weather) { currentWeather = data.weather; el('weather-display').textContent = currentWeather.name; }
...
function addUltCharge(amount) {
  ...
  el('ult-bar-fill').style.width = `${playerUltCharge}%`;
  el('btn-activate-ult').disabled = playerUltCharge < 100;
}

el('btn-activate-ult').onclick = () => {
  if (playerUltCharge < 100) return;
  playerUltCharge = 0; el('ult-bar-fill').style.width = `0%`; el('btn-activate-ult').disabled = true;
  const rect = el('enemy-avatar').getBoundingClientRect();
...
function playCard(index, currentData) {
  ...
  addUltCharge(15);
  const rectTarget = el('enemy-avatar').getBoundingClientRect();
...
el('tab-buy').onclick = () => { shopTabMode = 'buy'; setShopTabActive('tab-buy'); renderShop(); };
el('tab-relics').onclick = () => { shopTabMode = 'relics'; setShopTabActive('tab-relics'); renderShop(); };
el('tab-remove').onclick = () => { shopTabMode = 'remove'; setShopTabActive('tab-remove'); renderShop(); };
...
function openShop() {
  el('shop-coins').textContent = playerCoins;
    ...
    shopCards.forEach(item => {
      ...
      div.querySelector('.btn-buy').onclick = () => {
        if (playerCoins >= item.cost) {
          ...
          el('shop-coins').textContent = playerCoins; playSound('buy');
          saveCloudProgress(); renderShop();
        }
      };
      container.appendChild(div);
    });
...
function updateUI(data) {
  ...
  el('p1-name').textContent = me.name || (playerRole === 'p1' ? 'P1' : 'P2');
  el('p1-hp').textContent = me.hp; el('p1-max-hp').textContent = me.maxHp || 100;
  el('p1-shield').textContent = me.shield; el('p1-energy').textContent = me.energy;
  el('p1-max-energy').textContent = me.maxEnergy || 3; el('player-avatar').textContent = me.avatar || "🧙‍♂️";

  if (me.stun && Number(me.stun) > 0) el('p1-stun-wrap').classList.remove('hidden'); else el('p1-stun-wrap').classList.add('hidden');
  if (me.trap && Number(me.trap) > 0) el('p1-trap-wrap').classList.remove('hidden'); else el('p1-trap-wrap').classList.add('hidden');

  if (gameMode === 'solo') {
    el('p1-coins').textContent = playerCoins;
    el('player-relics-bar').innerHTML = playerRelics.map(rk => RELICS_DB[rk].icon).join(" ");
  }

  el('p2-name').textContent = opponent.name || (playerRole === 'p1' ? 'P2' : 'P1');
  el('p2-hp').textContent = opponent.hp; el('p2-max-hp').textContent = opponent.maxHp || 100;
  el('p2-shield').textContent = opponent.shield; el('enemy-avatar').textContent = opponent.avatar || "👺";

  if (opponent.poison && opponent.poison > 0) { el('p2-poison-wrap').classList.remove('hidden'); el('p2-poison').textContent = opponent.poison; } else el('p2-poison-wrap').classList.add('hidden');
  if (opponent.bleed && opponent.bleed > 0) { el('p2-bleed-wrap').classList.remove('hidden'); el('p2-bleed').textContent = opponent.bleed; } else el('p2-bleed-wrap').classList.add('hidden');
  if (opponent.stun && Number(opponent.stun) > 0) el('p2-stun-wrap').classList.remove('hidden'); else el('p2-stun-wrap').classList.add('hidden');
  if (opponent.trap && Number(opponent.trap) > 0) el('p2-trap-wrap').classList.remove('hidden'); else el('p2-trap-wrap').classList.add('hidden');

  el('reserve-slot-box').textContent = reservedCardKey ? CARDS_DB[reservedCardKey].name.substring(0, 2) : "➕";
  el('reserve-slot-box').onclick = () => {
